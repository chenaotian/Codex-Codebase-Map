...
v1 是 thread-id 风格的老式子 agent 控制面；v2 是 task-path + mailbox 风格的新协作模型。当前 turn 只要启用了 Feature::MultiAgentV2，正常多 agent 工具族就不会再注册 v1。默认 MultiAgentV2 是不打开的，需要自己主动配置。
...
只有用户明确要求 sub-agents、delegation 或 parallel agent work 时才由 当前父 agent 的模型发起。
...
用来等待一个或多个 v1 子 agent 中的任意一个进入“最终状态”。
...
当你执行close_agent(child)，它会做两件事：

1. 把 child 这个 agent 关掉
2. 把 child 下面活着的 grandchild 也一起关掉

但持久化状态里，它只把这条边标成 Closed：

```
parent -> child       Closed
child  -> grandchild  Open // 这里标记是open，但实际上已经关了
```

如果直接恢复 child，Codex 会先把 child 恢复出来。然后它继续看child 下面有哪些 Open 的 child？然后发现：

```
child -> grandchild 是 Open
```

于是 grandchild 会跟着恢复。也就是：

```
resume_agent(child)
=> 恢复 child
=> 因为 child -> grandchild 还是 Open
=> grandchild 也跟着恢复
```

如果后面parent也关闭了，然后resume parent，codex会先看parent 下面有哪些 Open 的 child？然后发现：

```
parent -> child 是 Closed
```

于是 child 不会被自动恢复，grandchild 当然也不会跟着恢复。也就是：

...
2. 先看有没有显式规则命中，显示规则类似，从配置文件加载，默认配置在`$CODEX_HOME/rules/default.rules`：
...
3. 没命中规则，再走默认启发式 fallback，没有规则，只有启发式代码，大概逻辑如下：
   ...
   ```
   1. 判断是不是 known safe command：白名单 + 参数安全检查
   2. 判断当前环境是否缺少 sandbox 保护
   3. 判断是不是 dangerous command：危险模式匹配 + 参数检查
   4. 根据 approval_policy 决定 allow / prompt / forbidden
   5. 如果是 on-request / granular，还看是否请求 sandbox override
   ```

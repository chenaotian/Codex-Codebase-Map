...
1. Session::steer_input
   位置：session/mod.rs (line 3122)
   写入内容：TurnInput::UserInput，以及可能附带的 additional_context 转成的 TurnInput::ResponseItem。
   典型来源：用户/agent 在当前 turn 运行中又提交了一条输入。

2. Session::inject_if_running / inject_no_new_turn
   位置：session/inject.rs (line 12)
   ...
   - thread/injectItems：codex 的修改/补充历史上下文的接口，有如下触发场景
     - 启动side conversation(侧边聊天/分支对话)的时候，注入SIDE_BOUNDARY_PROMPT
   ...
   - 保存用户的批准结果：
     ...
     - Guardian denied action：Guardian 自动安全审查先拒绝了某个具体动作，但用户后来手动批准了这个被拒绝的动作。这里只是注入上下文，后续模型可能会继续尝试调用这个命令。
